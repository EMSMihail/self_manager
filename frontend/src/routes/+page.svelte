<script>
  ...
  // 1. Загрузка: синхронизация с сервером и локальное отображение
  async function syncAndLoad() {
    try {
      const serverNotes = await fetchNotesFromBackend();
      await db.notes.clear();
      await db.notes.bulkAdd(serverNotes.map(n => ({ 
        ...n, 
        status: n.status || 'todo', 
        isSynced: 1 
      })));
    } catch (e) {
      console.warn("Бэкенд недоступен, переключено в оффлайн-режим", e);
    }
    await loadFromLocal();
  }
